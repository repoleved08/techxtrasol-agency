...
class PostResource extends Resource
{
  ...
  protected static function notifySubscribers(Post $post)
  {
    ...
    try {
      Notification::send($subscribers, new NewBlogPostNotification(
        $post->id,
        $recentPosts->pluck('id')->toArray()
      ));
    ...
    return $table
      ...
      ->actions([
        ...
        Tables\Actions\Action::make('notify')
          ...
          ->action(function (Post $record) {
            $recentPostIds = Post::where('is_published', true)
              ->where('id', '!=', $record->id)
              ->orderBy('published_at', 'desc')
              ->limit(3)
              ->pluck('id')
            ...
            Notification::send($subscribers, new NewBlogPostNotification($record->id, $recentPostIds));
